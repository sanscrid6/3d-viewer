import { useTexture } from '@react-three/drei'
import { useMemo } from 'react'
import { MeshBasicMaterial, Euler, MathUtils, type Vector3, Mesh, PlaneGeometry, TextureLoader, type Texture } from 'three'
import { isMaterial } from './utils'

// export function NavPoint ({ position, index }: { position: Vector3, index: number }) {
//   const texture = useTexture('circle.png')

//   const material = useMemo(() => {
//     return new MeshBasicMaterial({
//       map: texture,
//       transparent: true,
//       depthWrite: false,
//       depthTest: false,
//       opacity: 0.5,
//       polygonOffset: true,
//       polygonOffsetFactor: -1.0,
//       polygonOffsetUnits: -4.0
//     })
//   }, [texture])

//   return (
//     <mesh
//       material={material}
//       name={`Point${index}`}
//       position={position}
//       rotation={new Euler(MathUtils.degToRad(-90), 0, 0)}
//       scale={20}
//       userData={{ isNavPoint: true, index }}
//     >
//       <planeGeometry args={[1, 1]} />
//     </mesh>
//   )
// }

export interface IFadable {
  fadeIn: () => void
  fadeOut: () => void
}

export class NavPoint extends Mesh implements IFadable {
  static texture?: Texture

  public readonly isNavPoint = true

  constructor (position: Vector3) {
    const material = new MeshBasicMaterial({
      map: NavPoint.texture!,
      transparent: true,
      depthWrite: false,
      depthTest: false,
      opacity: 0.5,
      polygonOffset: true,
      polygonOffsetFactor: -1.0,
      polygonOffsetUnits: -4.0
    })
    super(new PlaneGeometry(1, 1), material)
    this.position.set(position.x, position.y, position.z)
    this.scale.set(20, 20, 20)
    this.rotation.set(MathUtils.degToRad(-90), 0, 0)
    this.name = 'NavPoint'
  }

  static async loadTexture () {
    const loader = new TextureLoader()
    this.texture = await loader.loadAsync('circle.png')
  }

  fadeIn () {
    if (isMaterial(this.material)) {
      this.material.opacity = 1
    }
  }

  fadeOut () {
    if (isMaterial(this.material)) {
      this.material.opacity = 0.5
    }
  }
}
